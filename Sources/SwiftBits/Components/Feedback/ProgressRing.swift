...
public struct ProgressRing<Label: View>: View {
    ...
    /// `accessibilityLabel` names the ring for VoiceOver; when `nil` a localized "Progress"
    /// provided by SwiftBits is used.
    public init(value: Double, tint: Color = .accentColor, lineWidth: CGFloat = 8,
                accessibilityLabel: LocalizedStringKey? = nil, @ViewBuilder label: () -> Label) {
    ...
    private var accessibilityText: Text {
        accessibilityTitle.map { Text($0) } ?? Text("Progress", bundle: .module)
    }
